...
_BUDGET_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\$\s*[5-9]\s*k|\$\s*[5-9][\s,]*000|[5-9]k", re.I), "$5k-$10k"),
    (re.compile(r"\$\s*[1-4]\s*k|\$\s*[1-4][\s,]*000|[1-4]k", re.I), "$1k-$5k"),
    (re.compile(r"\$\s*\d{2,3}(?!\d)|few hundred|small budget", re.I), "< $1k"),
]

_PLATFORM_KEYWORDS: dict[str, list[str]] = {
    "google": ["google", "google ads", "adwords", "gads", "pmax", "performance max"],
...
_URGENCY_PATTERNS: list[tuple[re.Pattern, str]] = [
    (re.compile(r"\basap\b|right now|immediately|urgent|today", re.I), "asap"),
    (re.compile(r"this week|next few days|in a couple days", re.I), "this_week"),
    (re.compile(r"this month|next week|soon|shortly", re.I), "this_month"),
