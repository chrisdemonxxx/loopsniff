...
class ConversationLoggerMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        # Wrap answer/reply so outgoing responses are logged automatically
        event.answer = _wrap_answer(event.answer, uid, uname, fname)
        event.reply = _wrap_answer(event.reply, uid, uname, fname)
...
class CallbackLoggerMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        if event.message and hasattr(event.message, "edit_text"):
            ...
            event.message.edit_text = logged_edit
