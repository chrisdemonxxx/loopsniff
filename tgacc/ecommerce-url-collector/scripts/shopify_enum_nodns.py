#!/usr/bin/env python3
"""
Massive myshopify.com subdomain enumeration - bypasses DNS by connecting
directly to Shopify's IP with Host header.
"""
import asyncio, aiohttp, csv, os, time, random, ssl
...
COMMON_WORDS = [
    ...
    "forward","general","gone","ground","happen","hold","hope","hour",
...
ECOM_WORDS = [
    ...
    "skin","care","glow","pure","organic","vegan","clean","fresh","aroma",
    ...
    "moon","star","sun","sky","ocean","wave",
    "wild","happy","lucky","magic","bliss",
    "pink","purple","coral","teal","ivory",
...
async def check_subdomain(session, subdomain, sem, results, stats):
    host = f'{subdomain}.myshopify.com'
    url = f'https://{SHOPIFY_IP}/products.json?limit=1'
    async with sem:
        try:
...
async def main():
    ...
    connector = aiohttp.TCPConnector(limit=120, ssl=sslctx, enable_cleanup_closed=True)
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for i in range(0, len(candidates), batch_size):
            ...
            if results:
                with open('output/shopify_massive_enum.csv','w',newline='') as f:
                    w = csv.DictWriter(f, fieldnames=['domain','myshopify','platform','has_products','product_count'])
                    w.writeheader()
                    w.writerows(results)
    elapsed = time.time() - stats['start']
    print(f"\nDone! {stats['checked']:,} checked in {elapsed:.0f}s, {stats['found']:,} found", flush=True)
    if results:
        with open('output/shopify_massive_enum.csv','w',newline='') as f:
            w = csv.DictWriter(f, fieldnames=['domain','myshopify','platform','has_products','product_count'])
            w.writeheader()
            w.writerows(results)
        print(f"Saved {len(results)} stores to output/shopify_massive_enum.csv", flush=True)
